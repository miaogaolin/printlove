...
```
如果再切换到 set 模式下生成，唯一不同点内容中第二行中的最后一个数字，set 模式下会是 1。

那 count 模式下为啥是 2 呢？

因为 `if sex == 1` 语句被执行力两次，看下图再说明下：

![](https://gitee.com/hxl_xsj/resource/raw/master/images/test-mode-count-html.png)
