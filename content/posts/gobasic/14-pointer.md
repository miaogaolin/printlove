...
在上篇文章中已经接触到了指针接收者的概念，这块简单说明一下，详细的请看看[《自定义类型和结构体 - 方法》](/posts/gobasic/13-type-2/)。
...
在了解[《内置集合 - 切片》](/posts/gobasic/8-切片/)这篇文章后应该明白切片的底层引用的是数组，在切片传递时不会改变底层数组的引用，但如果对切片进行追加操作后，数组引用就会改变。
